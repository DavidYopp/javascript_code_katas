...
describe('test for me to work with for practice', () => {
  test('test if Node is a class', () => {
    expect(typeof Node.prototype.constructor).toEqual('function');
  });


  test('testing the new size function', () => {
    const l = new List();
    expect(l.size()).toEqual(0);
    l.insertFirst(1);
    l.insertFirst(2);
  ...
  test('test the getLast Function works', ()=> {
    let l = new List();
    expect(l.getLast()).toEqual(null);
    l.insertFirst(1);
    expect(l.getFirst()).toEqual({data: 1, next: null});
    expect(l.getLast()).toEqual({data: 1, next: null});
    l.insertFirst(2);
    expect(l.getFirst()).toEqual({data: 2, next: {data: 1, next: null}});
    expect(l.getLast()).toEqual({data: 1, next: null});
  });
});
